...
To plot the area enter the expression \command{x>2 $\wedge$ x<7 $\wedge$ y>0 $\wedge∧$ y<⎮f(x)⎮} in the Algebra window and click the button \button{Plot} in the graphic window.
\end{indication}
...
\item Enter the expression \command{x>0 $\wedge$ x<2pi $\wedge$ y>MIN(f(x),g(x)) ∧ y<MAX(f(x),g(x))} in the Algebra window and click the button \button{Plot} in the graphic window.
\end{enumerate}
...
\end{enumerate}
To compute the volume of the solid of revolution enter the expression \command{INT(pi f(x)\^{}2, x, 0, 2)} and click the button \button{Simplify}.
\end{indication}
